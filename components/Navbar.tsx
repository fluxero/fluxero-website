      ...
      >
        ...
        <Link to="/" className="flex items-center gap-2 group" style={{ textDecoration: 'none' }}>
          <img
            ...
            onError={e => {
              ...
              el.style.cssText = 'font-family:"Barlow Condensed",sans-serif;font-weight:900;font-size:20px;letter-spacing:0.18em;color:#F1F5F9;';
        ...
        >
            ...
            return (
              <button
                ...
                style={{
                  ...
                  color: isActive ? '#3B82F6' : 'rgba(148,163,184,0.9)',
                  padding: scrolled ? '7px 16px' : '4px 0',
                  borderRadius: scrolled ? 999 : 0,
                  background: isActive && scrolled ? 'rgba(59,130,246,0.1)' : 'transparent',
                  border: 'none',
                  cursor: 'pointer',
                }}
                onMouseEnter={e => {
              ...
              >
                ...
                {!scrolled && (
                  <span className="absolute -bottom-1 left-0 h-px rounded-full transition-all duration-300"
                    style={{ background: '#3B82F6', width: isActive ? '100%' : '0%' }} />
                )}
              </button>
            );
          })}
          ...
          <Link
            ...
            style={{
              fontSize: 13,
              letterSpacing: '0.04em',
              color: isCalcPage ? '#3B82F6' : 'rgba(148,163,184,0.9)',
              padding: scrolled ? '7px 16px' : '4px 0',
              borderRadius: scrolled ? 999 : 0,
              background: isCalcPage && scrolled ? 'rgba(59,130,246,0.1)' : 'transparent',
              textDecoration: 'none',
            }}
          >
            Calculator
        ...
        <div className="hidden md:flex items-center gap-3">
          ...
          >
            ...
            <span className="font-mono text-[10px] tracking-[0.2em] uppercase" style={{ color: '#3B82F6' }}>Pilot Active</span>
          </div>
          <button
            ...
            onMouseEnter={e => {
              ...
              (e.currentTarget as HTMLElement).style.boxShadow = '0 0 32px rgba(59,130,246,0.45), inset 0 1px 0 rgba(255,255,255,0.15)';
            }}
            onMouseLeave={e => {
              (e.currentTarget as HTMLElement).style.transform = 'scale(1)';
              (e.currentTarget as HTMLElement).style.boxShadow = '0 0 20px rgba(59,130,246,0.2), inset 0 1px 0 rgba(255,255,255,0.1)';
            }}
          >
            Contact us
          </button>
        ...
        <button onClick={() => setOpen(!open)} className="md:hidden p-2" aria-label="Menu"
          ...
          <div className="flex flex-col gap-1.5">
            <span className="block h-px w-6 transition-all duration-300"
              style={{ background: '#F1F5F9', transform: open ? 'translateY(9px) rotate(45deg)' : 'none' }} />
            <span className="block h-px w-4 transition-all duration-300"
              style={{ background: '#3B82F6', opacity: open ? 0 : 1 }} />
            <span className="block h-px w-6 transition-all duration-300"
              style={{ background: '#F1F5F9', transform: open ? 'translateY(-9px) rotate(-45deg)' : 'none' }} />
          </div>
        </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            style={{ background: 'rgba(15,23,42,0.97)', backdropFilter: 'blur(20px)', borderTop: '1px solid rgba(59,130,246,0.08)' }}
          >
            <div className="px-6 py-8 flex flex-col gap-1">
              {links.map((l, i) => (
                <motion.button key={l.label}
                  ...
                  style={{
                    color: active === l.href.slice(1) ? '#3B82F6' : '#F1F5F9',
              ...
              <motion.div initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 }}>
                <Link
                  ...
                  style={{ color: isCalcPage ? '#3B82F6' : '#F1F5F9', textDecoration: 'none', borderBottom: '1px solid rgba(255,255,255,0.05)' }}
                >
