...
const Lbl: React.FC<{children:React.ReactNode;hint?:string;req?:boolean}> = ({children,hint,req}) => (
  <div style={{marginBottom:hint?3:8}}>
    <span style={{fontSize:19,fontWeight:600,color:C.white,display:"block",fontFamily:"'Syne',sans-serif"}}>
      {children}{req&&<span style={{color:C.green,marginLeft:4}}>*</span>}
    </span>
    {hint&&<div style={{fontSize:17,color:C.grey,marginTop:2,lineHeight:1.5}}>{hint}</div>}
  </div>
);
  ...
  <div>
    <div style={{display:"flex",alignItems:"center",background:C.bg3,border:`1.5px solid ${error?C.red:C.border}`,borderRadius:8,overflow:"hidden"}}>
      <input type={type} value={value} onChange={e=>onChange(e.target.value)} placeholder={placeholder}
        style={{flex:1,background:"none",border:"none",outline:"none",padding:"11px 14px",fontSize:19,color:C.white,fontFamily:"'IBM Plex Mono',monospace",boxSizing:"border-box"}}/>
      {unit&&<span style={{padding:"0 12px",color:C.grey,fontSize:17,borderLeft:`1px solid ${C.border}`,whiteSpace:"nowrap"}}>{unit}</span>}
    </div>
    {error&&<div style={{fontSize:17,color:C.red,marginTop:3}}>{error}</div>}
  </div>
);

const Sel: React.FC<{value:any;onChange:(v:string)=>void;options:{value:string;label:string}[]}> = ({value,onChange,options}) => (
  <select value={value} onChange={e=>onChange(e.target.value)}
    style={{width:"100%",background:C.bg3,border:`1.5px solid ${C.border}`,borderRadius:8,padding:"11px 14px",fontSize:19,color:C.white,outline:"none",cursor:"pointer",appearance:"none" as any}}>
...
const Sec: React.FC<{children:React.ReactNode}> = ({children}) => <div style={{fontSize:13,fontWeight:700,letterSpacing:"2px",color:C.greenD,marginBottom:12,marginTop:4,fontFamily:"'IBM Plex Mono',monospace"}}>{children}</div>;
  ...
  return (
    <span style={{position:"relative",display:"inline-block",marginLeft:6}}>
      <button
        ...
        onClick={()=>setShow(v=>!v)}
        style={{width:16,height:16,borderRadius:"50%",background:C.bg4,border:`1px solid ${C.border}`,color:C.grey,fontSize:16,cursor:"pointer",display:"inline-flex",alignItems:"center",justifyContent:"center",fontFamily:"serif"}}
      >?</button>
      {show&&<div style={{position:"absolute",left:"50%",bottom:"calc(100% + 8px)",transform:"translateX(-50%)",background:C.bg4,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 12px",width:220,fontSize:17,color:C.greyL,lineHeight:1.6,zIndex:100,boxShadow:"0 4px 20px rgba(0,0,0,0.4)"}}>
        {text}
...
const Toggle: React.FC<{checked:boolean;onChange:(v:boolean)=>void;label:string;sub?:string}> = ({checked,onChange,label,sub}) => (
  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",background:C.bg3,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 16px"}}>
    <div>
      <div style={{fontSize:19,fontWeight:600,color:C.white}}>{label}</div>
      {sub&&<div style={{fontSize:17,color:C.grey,marginTop:2}}>{sub}</div>}
    </div>
...
function S0_Source({value,onChange}: {value:string;onChange:(v:string)=>void}) {
  ...
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>What is your energy source?</h2>
      <p style={{fontSize:19,color:C.grey,marginBottom:24}}>Select the primary renewable source powering your hydrogen plant.</p>
      <div className="h2-grid-5" style={{display:"grid",gridTemplateColumns:"repeat(5,1fr)",gap:10}}>
        {opts.map(o=>(
          <button key={o.id} onClick={()=>onChange(o.id)} style={{padding:"18px 8px",border:`1.5px solid ${value===o.id?C.green:C.border}`,borderRadius:12,background:value===o.id?C.greenGlow:C.bg3,cursor:"pointer",outline:"none",textAlign:"center",transition:"all 0.2s"}}>
            ...
            <div style={{fontSize:18,fontWeight:700,color:value===o.id?C.green:C.white,fontFamily:"'Syne',sans-serif"}}>{o.label}</div>
            <div style={{fontSize:16,color:C.grey,marginTop:2}}>{o.sub}</div>
          </button>
        ))}
      </div>
    </div>
...
function S1_Location({data,onChange}: {data:any;onChange:(v:any)=>void}) {
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Where is your site?</h2>
      <p style={{fontSize:19,color:C.grey,marginBottom:22}}>We have built-in solar irradiance and wind speed data for every UK postcode area — no internet lookup needed.</p>
      ...
      <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:10,padding:"14px 16px"}}>
        <div style={{fontSize:17,fontWeight:700,color:C.greenD,marginBottom:8,fontFamily:"'IBM Plex Mono',monospace"}}>WHY POSTCODE MATTERS</div>
        <div style={{fontSize:17,color:C.grey,lineHeight:1.9}}>
...
function S2_Solar({data,onChange}: {data:any;onChange:(v:any)=>void}) {
  ...
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Solar Panel Configuration</h2>
      ...
      {Number(kWp)>0&&<div style={{background:C.greenGlow,border:`1px solid rgba(0,229,160,0.2)`,borderRadius:8,padding:"10px 14px",marginBottom:16,fontSize:18,color:C.green,fontWeight:600}}>⚡ System total: {kWp} kWp</div>}
...
function S3_Wind({data,onChange}: {data:any;onChange:(v:any)=>void}) {
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Wind Turbine Configuration</h2>
      <p style={{fontSize:19,color:C.grey,marginBottom:22}}>We use Weibull k=2 distribution integration across the full wind speed range — not just average speed — for accurate energy estimates.</p>
...
function S4_HydroLab({sourceType,hydro,lab,onHydro,onLab}: any) {
  if (sourceType==="hydro") return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Hydro Configuration</h2>
      <G2>
  ...
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Lab / PSU Configuration</h2>
      <G2>
...
function S5_Electrolyser({data,onChange,postcode,sourceType,solar,wind,hydro,lab}: any) {
  ...
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Electrolyser Configuration</h2>
      <p style={{fontSize:19,color:C.grey,marginBottom:22}}>We apply real published efficiency curves — not a flat number. Part-load behaviour and minimum load thresholds are fully modelled.</p>
      <Sec>TYPE & RATING</Sec>
      ...
      {suggestedKW&&(
        <div style={{background:C.greenGlow,border:`1px solid rgba(0,229,160,0.2)`,borderRadius:8,padding:"10px 14px",marginBottom:12,fontSize:18,color:C.green,fontWeight:600,cursor:"pointer"}} onClick={()=>onChange({...data,ratedKW:suggestedKW.toFixed(0)})}>
      ...
      {(data.type||"alkaline")==="alkaline"&&(
        <div style={{background:"rgba(255,184,0,0.07)",border:`1px solid rgba(255,184,0,0.2)`,borderRadius:10,padding:"12px 16px",marginBottom:16}}>
          <div style={{fontSize:17,fontWeight:700,color:C.amber,marginBottom:6}}>⚠ Alkaline minimum load: 20%</div>
          <div style={{fontSize:17,color:C.grey,lineHeight:1.7}}>
...
function S6_Revenue({data,onChange}: {data:any;onChange:(v:any)=>void}) {
  ...
  return (
    <div>
      <h2 style={{fontSize:24,fontWeight:800,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Revenue Model</h2>
      <p style={{fontSize:19,color:C.grey,marginBottom:20}}>How will the hydrogen be priced? This determines the revenue and payback calculation.</p>
      <div className="h2-grid-3" style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:10,marginBottom:20}}>
        {modes.map(m=>(
          <button key={m.id} onClick={()=>onChange({...data,mode:m.id})} style={{padding:"14px 10px",border:`1.5px solid ${data.mode===m.id?C.green:C.border}`,borderRadius:10,background:data.mode===m.id?C.greenGlow:C.bg3,cursor:"pointer",outline:"none",textAlign:"center",transition:"all 0.2s"}}>
            <div style={{fontSize:18,fontWeight:700,color:data.mode===m.id?C.green:C.white,marginBottom:4,fontFamily:"'Syne',sans-serif"}}>{m.label}</div>
            <div style={{fontSize:16,color:C.grey}}>{m.sub}</div>
          </button>
        ))}
      </div>
      {data.mode==="contract"&&(
...
const ChartTip: React.FC<any> = ({active,payload,label}) => {
  ...
  return (
    <div style={{background:C.bg4,border:`1px solid ${C.border}`,borderRadius:8,padding:"10px 14px"}}>
      <div style={{fontSize:17,fontWeight:700,color:C.white,marginBottom:4}}>{label}</div>
      {payload.map((p: any)=><div key={p.name} style={{fontSize:17,color:C.green}}>{p.name}: {fmtH2(p.value)}</div>)}
    </div>
  );
};
  ...
  return (
    <div className="h2-calc-shell" style={{display:"flex",height:"100%",overflow:"hidden",background:C.bg0}}>
      ...
      <div className="h2-calc-sidebar" style={{width:240,background:C.bg1,borderRight:`1px solid ${C.border}`,padding:"24px 20px",display:"flex",flexDirection:"column",flexShrink:0,overflowY:"auto"}}>
        ...
        <div style={{fontSize:22,fontWeight:800,color:C.white,marginBottom:4,fontFamily:"'Syne',sans-serif"}}>Results</div>
        <div style={{fontSize:17,color:C.grey,marginBottom:16}}>📍 {results.locationLabel}</div>
        {[
          ...
          <div key={d.label} style={{marginBottom:14}}>
            <div style={{fontSize:13,fontWeight:700,letterSpacing:"1.5px",color:C.greenD,marginBottom:3,fontFamily:"'IBM Plex Mono',monospace"}}>{d.label}</div>
            <div style={{fontSize:19,fontWeight:700,color:d.accent||C.white,fontFamily:"'IBM Plex Mono',monospace"}}>{d.val}</div>
          </div>
        ))}
        {(results.windCF!==null||results.solarYield!==null)&&(
          <div style={{background:C.bg2,border:`1px solid ${C.border}`,borderRadius:10,padding:"12px 14px",marginBottom:14}}>
            <div style={{fontSize:13,fontWeight:700,color:C.greenD,letterSpacing:"1px",marginBottom:8,fontFamily:"'IBM Plex Mono',monospace"}}>RESOURCE QUALITY</div>
            {results.windCF!==null&&<div style={{fontSize:17,color:C.grey,marginBottom:4}}>Wind CF: <b style={{color:C.white}}>{(results.windCF*100).toFixed(1)}%</b></div>}
            {results.solarYield!==null&&<div style={{fontSize:17,color:C.grey}}>Solar yield: <b style={{color:C.white}}>{results.solarYield.toFixed(0)} kWh/kWp/yr</b></div>}
          </div>
        )}
        <div style={{flex:1}}/>
        <button onClick={onBack} style={{width:"100%",padding:"10px",background:C.bg3,border:`1px solid ${C.border}`,borderRadius:8,color:C.greyL,fontSize:18,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>← Adjust inputs</button>
      </div>

      {/* Main */}
      <div className="h2-calc-main" style={{flex:1,overflowY:"auto",padding:"28px 32px",minWidth:0}}>
        <div style={{fontSize:22,fontWeight:800,color:C.white,marginBottom:4,fontFamily:"'Syne',sans-serif"}}>
          {fmtH2(results.annualH2Kg)} of green hydrogen per year
        </div>
        <div style={{fontSize:18,color:C.grey,marginBottom:16}}>{inputs.sourceType} · {results.locationLabel}</div>
        ...
        {results.warnings.map((w: any,i: number)=>(
          <div key={i} style={{padding:"12px 16px",background:"rgba(255,184,0,0.07)",border:`1px solid rgba(255,184,0,0.2)`,borderRadius:10,marginBottom:12,display:"flex",gap:10}}>
            ...
            <div style={{fontSize:17,color:C.grey,lineHeight:1.6}}>{w.msg}</div>
          </div>
        ))}
        ...
        <div className="h2-grid-4" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:14}}>
          {[
            ...
            <div key={c.lbl} style={{background:C.bg3,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 14px"}}>
              <div style={{fontSize:13,fontWeight:700,letterSpacing:"1.5px",color:C.grey,marginBottom:8,fontFamily:"'IBM Plex Mono',monospace"}}>{c.lbl}</div>
              <div style={{fontSize:26,fontWeight:800,color:c.ac,fontFamily:"'IBM Plex Mono',monospace",lineHeight:1}}>{c.val}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{display:"flex",gap:8,marginBottom:8}}>
          {[{id:"h2",lbl:"H₂ Output (kg)"},{id:"kwh",lbl:"Energy Input (MWh)"}].map(t=>(
            <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"6px 14px",border:`1px solid ${tab===t.id?C.green:C.border}`,borderRadius:6,background:tab===t.id?C.greenGlow:C.bg3,cursor:"pointer",color:tab===t.id?C.green:C.grey,fontSize:17,fontWeight:600,outline:"none"}}>
              {t.lbl}
            </button>
          ))}
        </div>
        <div style={{background:C.bg3,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 16px 8px",marginBottom:14}}>
          <ResponsiveContainer width="100%" height={160}>
            <BarChart data={chartData} margin={{top:4,right:4,bottom:0,left:-16}}>
              ...
              <XAxis dataKey="month" tick={{fill:C.grey,fontSize:16}} axisLine={false} tickLine={false}/>
              <YAxis tick={{fill:C.grey,fontSize:16}} axisLine={false} tickLine={false}/>
        ...
        <div style={{background:C.bg3,border:`1px solid ${C.border}`,borderRadius:12,padding:"16px 20px",marginBottom:14}}>
          ...
          {[
            ...
            <div key={k} style={{display:"flex",justifyContent:"space-between",padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
              <span style={{fontSize:18,color:C.grey}}>{k}</span>
              <span style={{fontSize:18,fontWeight:600,color:C.white,fontFamily:"'IBM Plex Mono',monospace"}}>{v}</span>
            </div>
          ))}
        </div>
        ...
        {results.aiInsights&&(
          <div style={{background:`linear-gradient(135deg,${C.bg3},rgba(0,229,160,0.04))`,border:`1px solid rgba(0,229,160,0.15)`,borderRadius:12,padding:"18px 20px",marginBottom:14}}>
            ...
            {results.aiInsights.farmerSummary&&(
              <div style={{fontSize:18,color:C.greyL,lineHeight:1.8,marginBottom:14,padding:"12px 14px",background:C.bg2,borderRadius:8,fontStyle:"italic"}}>
                "{results.aiInsights.farmerSummary}"
              </div>
            )}
            {results.aiInsights.insights?.map((ins: string,i: number)=>(
              <div key={i} style={{display:"flex",gap:8,marginBottom:8}}>
                ...
                <span style={{fontSize:17,color:C.greyL,lineHeight:1.6}}>{ins}</span>
              </div>
            ))}
            {results.aiInsights.recommendation&&(
              <div style={{marginTop:8,padding:"10px 12px",background:C.greenGlow,border:`1px solid rgba(0,229,160,0.15)`,borderRadius:8}}>
                <span style={{fontSize:17,fontWeight:700,color:C.green}}>✓ Recommendation: </span>
                <span style={{fontSize:17,color:C.grey}}>{results.aiInsights.recommendation}</span>
              </div>
            )}
          </div>
        )}
        ...
        <div style={{padding:"16px 20px",background:"rgba(0,229,160,0.04)",border:`1px solid rgba(0,229,160,0.14)`,borderRadius:12}}>
          <div style={{fontSize:18,fontWeight:700,color:C.green,marginBottom:4}}>🗺 Export to Fluxero Site Planner</div>
          <div style={{fontSize:17,color:C.grey,lineHeight:1.7,marginBottom:12}}>Copy this config JSON to the Unity site planner to visualise your installation on a 3D map.</div>
          <button onClick={()=>{navigator.clipboard.writeText(JSON.stringify(unityConfig,null,2));setCopied(true);setTimeout(()=>setCopied(false),2500);}}
            style={{padding:"9px 18px",background:copied?C.greenD:C.green,border:"none",borderRadius:8,color:C.bg0,fontSize:18,fontWeight:700,cursor:"pointer",transition:"background 0.2s",fontFamily:"inherit"}}>
...
function Loading({pct,msg}: {pct:number;msg:string}) {
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100%",background:C.bg0,padding:40}}>
      ...
      <div style={{fontSize:22,fontWeight:700,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Computing your site...</div>
      <div style={{fontSize:18,color:C.grey,marginBottom:24,textAlign:"center",minHeight:20}}>{msg}</div>
      ...
      <div style={{fontSize:17,color:C.grey,marginTop:8,fontFamily:"'IBM Plex Mono',monospace"}}>{pct}%</div>
    </div>
  );
}
        ...
        <div key={i} style={{display:"flex",alignItems:"center"}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}>
            <div style={{width:24,height:24,borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,transition:"all 0.2s",
              ...
              fontSize:16,fontWeight:700,color:i<cur?C.bg0:i===cur?C.green:C.grey,fontFamily:"'IBM Plex Mono',monospace"}}>
              {i<cur?"✓":i+1}
            </div>
            {i===cur&&<span style={{fontSize:17,color:C.white,fontWeight:600,fontFamily:"'Syne',sans-serif"}}>{s}</span>}
          </div>
  ...
  return (
    <div className="h2-calc-shell" style={{display:"flex",height:"100%",background:C.bg0}}>
      ...
      <div className="h2-calc-sidebar" style={{width:220,background:C.bg1,borderRight:`1px solid ${C.border}`,padding:"28px 20px",display:"flex",flexDirection:"column",position:"sticky",top:0,height:"100%",flexShrink:0}}>
        ...
        <div style={{fontSize:13,color:C.grey,marginBottom:28,fontFamily:"'IBM Plex Mono',monospace"}}>H₂ DESIGN ENGINE</div>
        <div style={{fontSize:19,fontWeight:700,color:C.white,marginBottom:6,fontFamily:"'Syne',sans-serif"}}>Site Calculator</div>
        <div style={{fontSize:17,color:C.grey,lineHeight:1.8,marginBottom:24}}>Postcode-specific UK data. Real physics. No internet required.</div>
        {["Every UK postcode area","NOABL wind speeds","Weibull k=2 wind model","IEC 61400 power curves","Alkaline/PEM efficiency curves","Auto electrolyser sizing","Oversizing warnings","AI site analysis"].map(f=>(
          <div key={f} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:6}}>
            ...
            <span style={{fontSize:16,color:C.greyL,lineHeight:1.4}}>{f}</span>
          </div>
        ))}
        <div style={{flex:1}}/>
        <div style={{fontSize:13,color:C.grey,marginBottom:4,fontFamily:"'IBM Plex Mono',monospace"}}>Step {step+1} of {content.length}</div>
      ...
      <div className="h2-calc-main" style={{flex:1,overflowY:"auto",padding:"36px 44px",maxWidth:740,minWidth:0}}>
        ...
        {Object.keys(errors).length>0&&(
          <div style={{background:"rgba(255,64,80,0.08)",border:`1px solid rgba(255,64,80,0.2)`,borderRadius:8,padding:"10px 14px",marginTop:16,fontSize:18,color:C.red}}>
            {Object.values(errors).join(" · ")}
          </div>
        )}
        <div style={{display:"flex",justifyContent:"space-between",marginTop:24,paddingTop:18,borderTop:`1px solid ${C.border}`}}>
          <button onClick={()=>step>0&&setStep(s=>s-1)} style={{padding:"11px 22px",background:step===0?"transparent":C.bg3,border:`1px solid ${step===0?"transparent":C.border}`,borderRadius:8,color:step===0?"transparent":C.greyL,fontSize:19,fontWeight:600,cursor:step===0?"default":"pointer",fontFamily:"inherit"}}>
            ← Back
          </button>
          <button onClick={()=>{if(!validate())return;isLast?doCalc():setStep(s=>s+1);}} style={{padding:"11px 28px",background:isLast?C.green:C.bg4,border:`1.5px solid ${isLast?C.green:C.border}`,borderRadius:8,color:isLast?C.bg0:C.white,fontSize:19,fontWeight:700,cursor:"pointer",fontFamily:"inherit",boxShadow:isLast?`0 0 20px rgba(0,229,160,0.25)`:"none"}}>
