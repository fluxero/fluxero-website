    ...
    <div className="max-w-7xl mx-auto px-6 lg:px-8 relative z-10">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-20">
        {[
          ...
          <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }}
            ...
            <p className="font-display font-bold text-cream text-base mt-3 mb-1 leading-snug">{s.label}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
        ...
        <motion.div initial={{ opacity: 0, x: -24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <p className="font-body text-mist text-base mb-8 font-light">Real engineering progress, confirmed milestones.</p>
          ...
          <div className="space-y-2">
            {milestones.map((m, i) => (
              <div key={i} className="flex items-center gap-4 rounded-lg px-4 py-3"
                ...
                <span className="font-body text-base flex-1" style={{ color: m.done ? '#F0EBE0' : '#64748B' }}>{m.label}</span>
                {!m.done && (
        ...
        <motion.div initial={{ opacity: 0, x: 24 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          ...
          <p className="font-body text-mist text-base mb-8 font-light">Institutions with access, expertise, and networks.</p>
            ...
            {partners.map((p, i) => (
              <div key={i} style={{ background: p.highlight ? 'rgba(0,214,143,0.05)' : '#131116', border: `1px solid ${p.highlight ? 'rgba(0,214,143,0.2)' : 'rgba(255,255,255,0.05)'}`, borderRadius: 10, padding: '24px 26px', display: 'flex', alignItems: 'flex-start', gap: 14 }}>
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-body font-semibold text-cream text-base">{p.name}</p>
      ...
      >
        <p className="font-mono text-xs tracking-[0.25em] uppercase mb-6" style={{ color: '#00D68F' }}>Why now</p>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div key={i}>
              ...
              <h4 className="font-display font-bold text-cream text-base mb-2">{item.title}</h4>
