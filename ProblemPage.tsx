...
export default function ProblemPage() {
  ...
  return (
    <div style={{ background: '#070608', minHeight: '100vh', color: '#F0EBE0' }}>
      ...
      {/* ── HERO ──────────────────────────────────────────────────── */}
      <div data-section style={{ padding: 'clamp(96px, 7vw, 116px) 2rem clamp(32px, 3vw, 44px)', textAlign: 'center', background: 'linear-gradient(to bottom, #070608, #0D0C0F)' }}>
        <motion.div {...fade}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 16, marginBottom: 22 }}>
            ...
            <span style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 17, letterSpacing: '0.25em', textTransform: 'uppercase', color: G }}>The Problem</span>
            <div style={{ width: 28, height: 1, background: G }} />
          </div>
          <h1 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(40px, 6vw, 80px)', fontWeight: 400, lineHeight: 1.05, marginBottom: 18, color: '#F0EBE0' }}>
          ...
          <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 'clamp(21px, 1.4vw, 23px)', fontWeight: 300, color: '#A8A3B3', maxWidth: 520, margin: '0 auto', lineHeight: 1.6 }}>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 18 }}>
          {STATS.map((s, i) => (
            <motion.div key={i} {...fade} transition={{ duration: 0.5, delay: i * 0.08 }}
              ...
              <div style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(38px, 3.6vw, 48px)', color: G, lineHeight: 1, marginBottom: 10 }}>{s.n}</div>
              <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 20, color: '#F0EBE0', fontWeight: 500, marginBottom: 6, lineHeight: 1.4 }}>{s.label}</p>
              <p style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 16, color: '#64748B', letterSpacing: '0.08em' }}>Source: {s.src}</p>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div data-section style={{ background: '#070608', padding: '3rem 2rem' }}>
        <div style={{ maxWidth: 1180, margin: '0 auto' }}>
          ...
          <motion.div {...fade} style={{ marginBottom: 28 }}>
            ...
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(32px, 4vw, 52px)', color: '#F0EBE0', marginBottom: 10, lineHeight: 1.1 }}>
              The waste is everywhere.
            </h2>
            <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 21, color: '#A8A3B3', fontWeight: 300, maxWidth: 620 }}>
              ...
              return (
                <div style={{ position: 'absolute', bottom: 16, left: 16, background: '#131116', border: `1px solid rgba(${r},${g},${b},0.35)`, borderRadius: 10, padding: '12px 16px', maxWidth: 280, boxShadow: '0 8px 32px rgba(0,0,0,0.4)' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    ...
                    <p style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 16, color: `rgba(${r},${g},${b},0.9)`, letterSpacing: '0.12em', textTransform: 'uppercase' }}>{KIND_LABEL[c.kind]}</p>
                  </div>
                  <p style={{ fontFamily: '"IBM Plex Sans Condensed", sans-serif', fontWeight: 700, fontSize: 19, color: '#F0EBE0', letterSpacing: '0.04em', marginBottom: 4 }}>{hovered}</p>
                  <p style={{ fontFamily: '"DM Serif Display", serif', fontSize: 28, color: '#F0EBE0', lineHeight: 1, marginBottom: 6 }}>{c.twh} TWh</p>
                  <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 18, color: '#94A3B8', lineHeight: 1.4 }}>{c.note}</p>
                </div>
              );
            })()}
          </motion.div>
          ...
          {/* Source line */}
          <p style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 16, color: '#334155', marginTop: 10, textAlign: 'right' }}>
          ...
          <motion.div {...fade} transition={{ duration: 0.5, delay: 0.15 }}
            ...
            <p style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 16, color: '#64748B', letterSpacing: '0.18em', textTransform: 'uppercase', padding: '0 16px', marginBottom: 18 }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              {ranked.map((c, i) => {
                ...
                return (
                  ...
                  >
                    <span style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 17, color: '#475569', textAlign: 'right' }}>{String(i + 1).padStart(2, '0')}</span>
                    <span style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 20, color: '#F0EBE0', fontWeight: 400, letterSpacing: '0.01em' }}>{c.name}</span>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, justifyContent: 'flex-end' }}>
                      ...
                      <span style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 18, color: '#F0EBE0', fontVariantNumeric: 'tabular-nums' }}>{c.twh} TWh</span>
                    </div>
                  </div>
                );
              })}
      ...
      <div data-section style={{ background: '#0D0C0F', padding: '3rem 2rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <motion.div {...fade} style={{ marginBottom: 28 }}>
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(32px, 4vw, 52px)', color: '#F0EBE0', lineHeight: 1.1, marginBottom: 10 }}>
              Hydrogen is the answer.
            </h2>
            <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 21, color: '#A8A3B3', fontWeight: 300, maxWidth: 520 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 14 }}>
            {[
              ...
              <motion.div key={i} {...fade} transition={{ duration: 0.4, delay: i * 0.07 }}
                ...
                <div style={{ fontFamily: '"DM Serif Display", serif', fontSize: 30, color: G, marginBottom: 6, lineHeight: 1 }}>{item.v}</div>
                <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 19, color: '#F0EBE0', fontWeight: 500, marginBottom: 4 }}>{item.l}</p>
                <p style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 16, color: '#64748B' }}>{item.s}</p>
              </motion.div>
            ))}
          </div>
        </div>
        ...
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <motion.div {...fade} style={{ marginBottom: 32 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
              <div style={{ width: 28, height: 1, background: G }} />
              <span style={{ fontFamily: '"IBM Plex Mono", monospace', fontSize: 17, letterSpacing: '0.25em', textTransform: 'uppercase', color: G }}>Timing</span>
            </div>
            <h2 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 'clamp(32px, 4vw, 52px)', color: '#F0EBE0', lineHeight: 1.1 }}>
              Why now.
            </h2>
          </motion.div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 18 }}>
            {[
              ...
              <motion.div key={i} {...fade} transition={{ duration: 0.5, delay: i * 0.08 }}
                ...
                <h3 style={{ fontFamily: '"DM Serif Display", serif', fontSize: 22, color: '#F0EBE0', marginBottom: 8 }}>{w.h}</h3>
                <p style={{ fontFamily: '"IBM Plex Sans", sans-serif', fontSize: 19.5, color: '#A8A3B3', lineHeight: 1.6, fontWeight: 300 }}>{w.s}</p>
              </motion.div>
            ))}
          </div>
        </div>
